...
function WeightChart({ data, w=300, h=100, visible=true }) {
  ...
  return (
    <svg ref={svgRef} width="100%" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none"
      ...
      {pts.map((p,i)=><circle key={i} cx={p.x} cy={p.y} r={hover?.idx===i?3:1.5}
        fill={hover?.idx===i?C.mint:'rgba(184,255,87,0.25)'}
        stroke={hover?.idx===i?'rgba(255,255,255,0.4)':'none'} strokeWidth={0.8}
        style={{transition:'r .12s, fill .12s'}}>
        {i===pts.length-1&&!hover&&<animate attributeName="r" values="1.5;2.5;1.5" dur="2.5s" repeatCount="indefinite"/>}
      </circle>)}
      {hover&&(()=>{const bw=50,bh=18,tx=Math.min(Math.max(hover.x,bw/2+2),w-bw/2-2),ty=Math.max(6,hover.y-26);return<>
        <line x1={hover.x} y1={padT} x2={hover.x} y2={h-padB} stroke={C.mint} strokeWidth="0.4" strokeDasharray="2,3" opacity="0.25"/>
        <rect x={tx-bw/2} y={ty} width={bw} height={bh} rx={5} fill={C.surfaceSolid} fillOpacity="0.92" stroke="rgba(184,255,87,0.1)" strokeWidth="0.5"/>
        <text x={tx} y={ty+8} textAnchor="middle" style={{fontSize:7.5,fill:C.mint,fontFamily:'var(--mono)',fontWeight:600}}>{data[hover.idx].kg}kg</text>
        <text x={tx} y={ty+15} textAnchor="middle" style={{fontSize:5.5,fill:C.text3,fontFamily:'var(--mono)'}}>W{data[hover.idx].week} · {data[hover.idx].date}</text>
      </>})()}
    </svg>
  );
}
...
Be concise (2-4 sentences), practical, reference actual numbers. Suggest specific foods with macros when relevant.`;
  ...
  const getOfflineTip = (q) => {
    ...
    if (ql.includes("eat")||ql.includes("dinner")||ql.includes("meal")||ql.includes("food"))
      return `You still need ~${remaining.cal} cal and ${remaining.pro}g protein. Try: 150g salmon (280cal, 30g pro) + 200g broccoli (68cal, 6g fiber) + 100g lentils (116cal, 9g pro, 8g fiber). That closes your macro gaps nicely.`;
    ...
    if (ql.includes("step")||ql.includes("walk"))
      return `${today.steps.toLocaleString()} steps today. ${today.steps>=8000?"Target hit!":"Need "+remaining.steps+" more. A brisk 20-min walk adds ~2,500 steps."}`;
    if (ql.includes("weight")||ql.includes("progress"))
      return `You've lost ${lost}kg in ${lastW.week+1} weeks (${lostPct}% of starting weight). At -${insights.velocity}kg/week, you're on pace to reach 68kg in ~${Math.ceil((lastW.kg-68)/parseFloat(insights.velocity||0.8))} more weeks.`;
    if (ql.includes("sleep"))
      return `${today.sleep}h sleep. ${today.sleep>=7?"Good!":"Below 7h. Poor sleep raises cortisol and stalls fat loss. Try: no screens 30min before bed, consistent bedtime."}`;
    return `Today: ${today.cal} cal (${today.cal>=1300&&today.cal<=1500?"on target":"needs attention"}), ${today.pro}g protein (${today.pro>=130?"good":"low"}), ${today.fib}g fiber (${today.fib>=20?"ok":"add veg"}), ${today.steps} steps (${today.steps>=8000?"hit":"need more"}). Streak: ${insights.streak} days. Keep pushing.`;
  };
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const reply = data.content?.map(i=>i.text||"").join("") || getOfflineTip(text);
